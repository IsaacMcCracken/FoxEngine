...
key_callback :: proc "c" (window: glfw.WindowHandle, key, scancoder, action, mods: c.int) {
  ...
  keyboard.current[key] = Input_Action(action)
}

is_key_down :: proc(k: Keyboard_Key) -> bool {
  return glfw.GetKey(app.window, c.int(k)) > glfw.RELEASE
}


is_key_pressed :: proc(k: Keyboard_Key)-> bool {
  i := u16(k)
  return  keyboard.current[i] == .Press && (keyboard.current[i] != keyboard.previous[i])
}